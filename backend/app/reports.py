...
def _ai_provider_label(ai_proposal: dict) -> str:
    ...
    if status == "mock" or identity == PROVIDER_MOCK:
        return "Mock Fallback"
    if identity == PROVIDER_AZURE:
...
def _analysis_attempt_lines(snapshot: dict) -> str:
    if not snapshot.get("analysis_attempts_available"):
        return "* Analysis attempt history is unavailable for this case."

    attempts = snapshot.get("analysis_attempts")
    if not isinstance(attempts, list) or not attempts:
        return "* No analysis attempt records were stored for this case."

    lines = []
    for attempt in attempts:
        if not isinstance(attempt, dict):
            continue
        timestamp = _format_timestamp(attempt.get("created_at"))
        status = _safe(attempt.get("status"), "unknown")
        provider = _safe(attempt.get("provider"), "Not available")
        diagnostic = attempt.get("diagnostic")
        suffix = " Labelled mock fallback." if status == "mock" else ""
        diagnostic_text = (
            f" Diagnostic: {_safe(diagnostic)}." if diagnostic else ""
        )
        lines.append(
            f"* {timestamp}: **{status}** via **{provider}**."
            f"{suffix}{diagnostic_text}"
        )
    return "\n".join(lines) if lines else (
        "* No valid analysis attempt records were stored for this case."
    )
...
def generate_report(case) -> str:
    ...
    analysis_attempt_lines = _analysis_attempt_lines(snapshot)
...
### Analysis Attempt History

{analysis_attempt_lines}
